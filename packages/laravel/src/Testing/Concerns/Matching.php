  ...
  public function where(string $key, $expected): self
  {
    ...
    if ($expected instanceof Closure) {
      PHPUnit::assertTrue(
        ...
        sprintf(
          'Navigare property [%s] was marked as invalid using a closure.',
          $this->dotPath($key)
        )
      );
    ...
    if ($expected instanceof Arrayable) {
      ...
      $expected instanceof ResourceResponse ||
      $expected instanceof JsonResource
    ) {
      $expected = json_decode(
        json_encode($expected->toResponse(request())->getData()),
        true
      );
    }
    ...
    PHPUnit::assertSame(
      ...
      $actual,
      sprintf(
        'Navigare property [%s] does not match the expected value.',
        $this->dotPath($key)
      )
    );
