...
trait Has
{
  protected function count(string $key, int $length): self
  {
    PHPUnit::assertCount(
      ...
      $this->prop($key),
      sprintf(
        'Navigare property [%s] does not have the expected size.',
        $this->dotPath($key)
      )
    );
  ...
  public function has(string $key, $value = null, Closure $scope = null): self
  {
    PHPUnit::assertTrue(
      ...
      sprintf('Navigare property [%s] does not exist.', $this->dotPath($key))
    );
    ...
    if (is_int($value) && !is_null($scope)) {
      ...
      PHPUnit::assertTrue(
        $value > 0,
        sprintf(
          'Cannot scope directly onto the first entry of property [%s] when asserting that it has a size of 0.',
          $path
        )
      );
      PHPUnit::assertIsArray(
        $prop,
        sprintf(
          'Direct scoping is currently unsupported for non-array like properties such as [%s].',
          $path
        )
      );
  ...
  public function missing(string $key): self
  {
    ...
    PHPUnit::assertNotTrue(
      Arr::has($this->prop(), $key),
      sprintf(
        'Navigare property [%s] was found while it was expected to be missing.',
        $this->dotPath($key)
      )
    );
  ...
  {
    return $this->missingAll(is_array($key) ? $key : func_get_args());
  }
