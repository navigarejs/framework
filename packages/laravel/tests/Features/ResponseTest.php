...
class ResponseTest extends TestCase
{
  ...
  public function test_server_response(): void
  {
    ...
    $this->assertSame('Julian', $page['props']['user']['name']);
    $this->assertSame('/user/123', $page['url']);
    $this->assertSame('123', $page['version']);
    $this->assertSame(
      '<div id="app" data-page="{&quot;component&quot;:&quot;User\/Edit&quot;,&quot;props&quot;:{&quot;user&quot;:{&quot;name&quot;:&quot;Julian&quot;}},&quot;url&quot;:&quot;\/user\/123&quot;,&quot;version&quot;:&quot;123&quot;}"></div>',
      $view->render()
    );
  }
  ...
  public function test_resource_response(): void
  {
    ...
    $resource = new FakeResource(['name' => 'Julian']);
  ...
  public function test_lazy_resource_response(): void
  {
    ...
    tap($page->props->users, function ($users) use ($expected) {
      $this->assertSame(
        json_encode($expected['data']),
        json_encode($users->data)
      );
      $this->assertSame(
        json_encode($expected['links']),
        json_encode($users->links)
      );
  ...
  public function test_nested_lazy_resource_response(): void
  {
    ...
    $callable = static function () use ($users) {
      ...
      return [
        'users' => new class ($page, JsonResource::class) extends
          ResourceCollection
        {
        },
      ];
    ...
    tap($page->props->something->users, function ($users) use ($expected) {
      $this->assertSame(
        json_encode($expected['users']['data']),
        json_encode($users->data)
      );
      $this->assertSame(
        json_encode($expected['users']['links']),
        json_encode($users->links)
      );
      $this->assertSame('/', $users->meta->path);
    });
  ...
  public function test_arrayable_prop_response(): void
  {
    ...
    $resource = FakeResource::make(['name' => 'Julian']);
  ...
  public function test_xhr_partial_response(): void
  {
    $request = Request::create('/user/123', 'GET');
    $request->headers->add(['X-Navigare' => 'true']);
    $request->headers->add(['X-Navigare-Partial-Component' => 'User/Edit']);
    $request->headers->add(['X-Navigare-Partial-Data' => 'partial']);
  ...
  public function test_lazy_props_are_included_in_partial_reload(): void
  {
    $request = Request::create('/users', 'GET');
    $request->headers->add(['X-Navigare' => 'true']);
    $request->headers->add(['X-Navigare-Partial-Component' => 'Users']);
    $request->headers->add(['X-Navigare-Partial-Data' => 'lazy']);
  ...
  public function test_top_level_dot_props_get_unpacked(): void
  {
    ...
    $this->assertSame('Julian Hundeloh', $user['name']);
  ...
  public function test_nested_dot_props_do_not_get_unpacked(): void
  {
    ...
    $this->assertSame('Julian Hundeloh', $auth['user']['name']);
